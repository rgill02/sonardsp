...
def gravity(lat):
    """Calculate gravity g(Φ) at latitude Φ in degrees."""
    phi = math.radians(lat)
    return 9.780318 * (1 + 5.2788e-3 * math.sin(phi) ** 2 + 2.36e-5 * math.sin(phi) ** 4)


def depth_from_pressure(P, lat):
    """
    Convert pressure P (kPa) to depth Z (meters) at latitude lat (degrees)
    using Leroy and Parthiot (1998) formula for standard ocean.

    Parameters
    ----------
    P : float
        Pressure in kPa (relative to atmospheric)
    lat : float
        Latitude in degrees
    ...
    """
    g_phi = gravity(lat)
    P_MPa = P / 1000  # convert kPa to MPa
    numerator = (9.72659e2 * P_MPa
                 - 2.2512e-1 * P_MPa ** 2
                 + 2.279e-4 * P_MPa ** 3
                 - 1.82e-7 * P_MPa ** 4)
    denominator = g_phi + 1.092e-4 * P_MPa
    Z = numerator / denominator
    return Z


def pressure_from_depth(Z, lat):
    """
    Convert depth Z (meters) to pressure P (kPa) at latitude lat (degrees)
    using Leroy and Parthiot (1998) formula for standard ocean.
    ...
    g_phi = 9.7803 * (1 + 5.3e-3 * sin_phi2)

    def h_Z_45(z):
        return (1.00818e-2 * z + 2.465e-8 * z ** 2 - 1.25e-13 * z ** 3 + 2.8e-19 * z ** 4)

    def k_Z_phi(z, phi):
        return (g_phi - 2e-5) / (9.80612 - 1e-5 * z)

    def h_Z_phi(z, phi):
        return h_Z_45(z) * k_Z_phi(z, phi)

    def h0_Z(z):
        return 1.0e-2 * z / (z + 100) + 6.2e-6 * z

    P_MPa = h_Z_phi(Z, phi) - h0_Z(Z)
    P_kPa = P_MPa * 1000
    return P_kPa
...
def c_mackenzie(T, z, S=35):
    ...
    # Compute sound speed
    return 1448.96 + 4.591 * T - 5.304e-2 * (T ** 2) + 2.374e-4 * (T ** 3) + 1.340 * (
                S - 35) + 1.630e-2 * z + 1.675e-7 * (z ** 2) - 1.025e-2 * T * (S - 35) - 7.139e-13 * T * (z ** 3)
...
def c_coppens(T, z, S=35):
    ...
    # Compute sound speed
    t = T / 10
    c_surface = 1449.05 + 45.7 * t - 5.21 * (t ** 2) + 0.23 * (t ** 3) + (1.333 - 0.126 * t + 0.009 * (t ** 2)) * (
                S - 35)
    return c_surface + (16.23 + 0.253 * t) * z + (0.213 - 0.1 * t) * (z ** 2) + (0.016 + 0.0002 * (S - 35)) * (
                S - 35) * t * z
...
if __name__ == "__main__":
    # Testa a gravidade na latitude 45°
    print("Gravity at 45°:", gravity(45))

    # Testa a conversão pressão -> profundidade
    pressure_kPa = 2000  # exemplo de pressão
    lat = 45
    depth = depth_from_pressure(pressure_kPa, lat)
    print(f"Depth from pressure {pressure_kPa} kPa at latitude {lat}°: {depth:.2f} m")

    # Testa a conversão profundidade -> pressão
    depth_m = 1000  # exemplo de profundidade
    pressure = pressure_from_depth(depth_m, lat)
    print(f"Pressure from depth {depth_m} m at latitude {lat}°: {pressure:.2f} kPa")
